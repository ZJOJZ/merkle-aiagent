...
def test_trading_actions_generation():
    ...
    market_data = get_extended_market_data()
    logger.info(f"Generated market data for {len(market_data)} symbols")

    # Log the market data we're using
    for symbol_data in market_data:
        logger.info(
            f"Symbol: {symbol_data['symbol']}, Price: {symbol_data['price']}, Change: {symbol_data['priceChange24h']}%"
        )
