    ...
    const onClickButton = async () => {
        ...
        try {
          ...
          const executedTransaction = await aptosClient().waitForTransaction({transactionHash: committedTransaction.hash,});
            ...
            if(n4 > 10_000_000n){
                const transaction = await OpenPosition("SOL_USD", n4, islong4, lever4, account.address, merkle);
                const committedTransaction = await signAndSubmitTransaction(transaction);
                const executedTransaction = await aptosClient().waitForTransaction({transactionHash: committedTransaction.hash,});
            }
            const n5 = BigInt(Math.floor(amount5) * totalinput) * 10_000n;
            if(n5 > 10_000_000n){
                const transaction = await OpenPosition("OP_USD", n5, islong5, lever5, account.address, merkle);
                const committedTransaction = await signAndSubmitTransaction(transaction);
                const executedTransaction = await aptosClient().waitForTransaction({transactionHash: committedTransaction.hash,});
            }
            const n6 = BigInt(Math.floor(amount6) * totalinput) * 10_000n;
            if(n6 > 10_000_000n){
                const transaction = await OpenPosition("SUI_USD", n6, islong6, lever6, account.address, merkle);
                const committedTransaction = await signAndSubmitTransaction(transaction);
                const executedTransaction = await aptosClient().waitForTransaction({transactionHash: committedTransaction.hash,});
            }
